...
import { Users, Award, Calendar, TrendingUp, Plus, ChevronRight, Shield, MapPin, Clock } from "lucide-react";
...
const StatsCard = ({ title, value, icon: Icon, gradient, isLoading, linkTo }) => {
  ...
  return (
    <div 
      onClick={handleClick}
      className="group relative overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer bg-white rounded-2xl p-6"
    >
      ...
      <div className="relative">
        <div className="flex justify-between items-start mb-4">
          ...
          <ChevronRight className="w-5 h-5 text-slate-400 opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-1 transition-all duration-300" />
        </div>
...
const PatrolBreakdown = ({ scouts, isLoading }) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-2xl shadow-lg border-0 p-6 animate-pulse">
            ...
            return (
              <div key={patrol}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-semibold text-slate-700">{patrol}</span>
                  <span className="text-sm font-bold text-slate-900">{count} scouts</span>
                </div>
                <div className="h-10 bg-slate-100 rounded-xl overflow-hidden relative">
                  <div
                    className={`h-full bg-gradient-to-r ${patrolColors[patrol] || patrolColors.Unassigned} transition-all duration-1000 ease-out flex items-center justify-end pr-3`}
          ...
          {activities.length > 0 ? (
            activities.map((activity, index) => (
              <div key={index} className="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-900 to-blue-700 rounded-full flex items-center justify-center flex-shrink-0">
      ...
      <div className="p-6 border-b border-slate-100">
        <div className="flex justify-between items-center">
          ...
          <button 
            ...
            className="text-blue-900 hover:text-blue-700 font-semibold text-sm flex items-center gap-1 transition-colors"
          >
            View All
        ...
        <div className="space-y-4">
          {upcomingEvents.length > 0 ? (
            upcomingEvents.map((event) => (
              <div key={event.id} className="group p-4 border border-slate-200 rounded-xl hover:border-blue-300 hover:shadow-md transition-all duration-300 bg-slate-50 hover:bg-white cursor-pointer">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-900 group-hover:text-blue-900 transition-colors mb-2">
                      {event.title}
                    </h3>
                  ...
                  <span className={`${typeColors[event.event_type] || typeColors.Other} text-xs px-2 py-1 rounded-full font-medium whitespace-nowrap ml-2`}>
                    {event.event_type}
                  </span>
                </div>
                {event.location && (
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-600">Here's what's happening with Troop {activeTroop} today</p>
          </div>
          <button 
            onClick={() => navigate('/app/scouts')}
            className="bg-gradient-to-r from-blue-900 to-blue-700 hover:from-blue-800 hover:to-blue-600 text-white font-semibold rounded-xl shadow-lg px-6 py-3 transition-all duration-200 hover:scale-105 hover:shadow-xl flex items-center gap-2 w-fit"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            {/* Quick Actions Card */}
            <div className="bg-gradient-to-br from-blue-900 to-blue-700 rounded-2xl p-6 shadow-lg text-white">
              <h3 className="text-lg font-bold mb-4">Quick Actions</h3>
              <div className="space-y-3">
                <button 
                  onClick={() => navigate('/app/events')}
                  className="w-full bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-xl p-3 text-left transition-all duration-200 flex items-center justify-between group"
                >
                ...
                <button 
                  ...
                  className="w-full bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-xl p-3 text-left transition-all duration-200 flex items-center justify-between group"
                >
                  <span className="font-medium">Award Badge</span>
                  <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </button>
                <button 
                  onClick={() => navigate('/app/events')}
